...
export function validateConfig(config: string): boolean {
  ...
  return {
    ...
    files: Object.keys(sourceFiles).reduce((acc, key) => {
...
describe('ASTDependencyAnalyzer - Full Pipeline', async () => {
  test('should analyze class with inheritance and implementation', async (t) => {
    ...
    try {
      ...
      assert.ok(baseService?.dependencies.has('MetricsCollector'), 'BaseService should depend on MetricsCollector');
  
      const logger = result.dependencies.get('Logger');
      assert.ok(logger?.dependencies.has('MetricsCollector'), 'Logger should depend on MetricsCollector');
  
      // Re-exported dependencies
      const metricsCollector = baseService?.dependencies.get('MetricsCollector');
      assert.ok(metricsCollector?.dependencies.has('MetricsConfig'), 'MetricsCollector should depend on MetricsConfig');
  
      // Verify source code chunks
      assert.ok(!result.sourceCode.includes('validateConfig'), 'Should not include unrelated code');
      assert.ok(logger?.sourceCode.includes('class Logger'), 'Should extract Logger class definition');
    } finally {
